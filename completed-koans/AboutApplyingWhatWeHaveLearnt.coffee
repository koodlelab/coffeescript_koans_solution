...
describe 'About Applying What We Have Learnt', ->
  ...
  it 'should find needle in a haystack (functional)', ->
    ...
    findNeedle = (ops) ->
      hasInvalidOperation = (a) -> if a.direction == 'FWD' && a.distance > 100 then true else false
      ops.filter hasInvalidOperation
    findNeedle(operations).should.deep.equal [{direction: 'FWD', distance: 200}]
  ...
  it 'should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)', ->
    ...
    # HINT: one way of doing this would be a 'for in when' construct using sum and
    # filter functions
    naturalNumber = (a) -> if a % 3 == 0 || a % 5 == 0 then return true else false
    naturalNumbers = [1..1000].filter naturalNumber
    total = (i, a) -> i + a
    nTotal = naturalNumbers.reduce total
    nTotal.should.equal 234168
  ...
  it 'should find the sum of all the even valued terms in the fibonacci sequence which do not exceed four million (functional)', ->
    ...
    even = (a) -> if a % 2 == 0 then return true else false
    fib = [0, 1, 1]
    i = 3
    while (fib[i] = fib[i-1] + fib[i-2]) < 4000000
      i+=1
    fib2 = fib.filter even
    console.log fib2
    sum = fib2.reduce (i, a) -> i + a
    sum.should.equal 4613732
